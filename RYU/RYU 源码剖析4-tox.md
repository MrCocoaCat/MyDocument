###  environments
指定创建的虚拟环境(virtualenv Environments)
当使用tox 命令时同时创建，使用-e 可以指定单个环境
```
[tox]
envlist = py27,py34,py35,py36,py37,pypy,pycodestyle,autopep8
```
### specifying a platform
{toxinidir} (the directory where tox.ini resides) 即当前工作目录
```
[testenv]
# 指定pip-requires 为requirements.txt
deps =
  -U
